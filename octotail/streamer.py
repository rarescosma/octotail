...
def run_streamer(ws_sub: WsSub, queue: mp.Queue[StreamerMsg]) -> mp.Process:
    process = mp.Process(target=_streamer, args=(ws_sub, queue))
    process.start()
    return process


def _streamer(ws_sub: WsSub, queue: mp.Queue[StreamerMsg]) -> None:
...
async def _stream_it(ws_sub: WsSub, queue: mp.Queue[StreamerMsg]) -> None:
