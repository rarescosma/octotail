...
class BrowserWatcher(ThreadingActor):
    ...
    opts: Opts
    inbox: mp.Queue[BrowseRequest]
    mgr: ActorRef[Manager]

    def __init__(self, mgr: ActorRef[Manager], opts: Opts, inbox: mp.Queue[BrowseRequest]):
        super().__init__()
        self.opts = opts
        self.inbox = inbox
        self.mgr = mgr
...
def _start_controller(opts: Opts, inbox: mp.Queue[BrowseRequest]) -> None:
